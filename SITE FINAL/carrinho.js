...
function addCarrinho(){
    var notebook = {nome: "Notebook Acer Swift 3 SF314-57-57VY Intel Core I5 16GB 256GB SSD 14' Windows 10", valor:"2095.60"}
...
function exibirProdutos(){
    ...
    listaProdutos.forEach(item => {
        ...
        codHtml += `<div class= "teste1"><img class="imagemNotebook" src="images copy/notebook completa.png" alt=""/><p>${item.nome} <br> (${qtd}) </p><b> <a href="javascript:void(0)"onclick="deletar(${index})"><img class="paraExcluir" src="./images copy/iconExcluir.png" alt=""></a></b></div>`
...
function total(){
    ...
    listaProdutos.forEach(item => {
        ...
        total += valor
        parcela = valor/4
        printTotal = `<div><h1>R$${total},00</h1><p>ou 4x de R$${parcela}</p></div>`
        
    });

    if(total == 0){
        document.getElementById('valorTotal').innerHTML ='<h2>Seu carrinho está vazio</h2>'
        document.getElementById('valorTotal').innerHTML ='<h2>Seu carrinho está vazio</h2>'
    }
    else{
